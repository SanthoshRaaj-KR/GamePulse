...
const Sidebar = ({ isExpanded, setIsExpanded }) => {
  return (
    <div
      className={`h-screen bg-black text-white flex flex-col gap-6 py-6 px-4 transition-all duration-300 ${
...
const SidebarItem = ({ icon, label, isExpanded }) => (
  <div className="flex items-center group cursor-pointer transition-all relative">
    <div
      className={`flex items-center justify-center p-2 rounded-full group-hover:bg-white/10 transition-all ${
        isExpanded ? "" : "mx-auto"
      }`}
    >
      {icon}
    ...
    {isExpanded ? (
      ...
      <div className="absolute left-16 bg-gray-800 text-white px-3 py-2 rounded shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-10">
        {label}
      </div>
    )}
  </div>
